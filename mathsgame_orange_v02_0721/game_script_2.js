...
function initClawMachine() {
  ...
  const lowerRodLength = 30;
    ...
    const maxUpperAngle = Math.PI / 4; // 45 degrees
...
function spawnFinalVictoryBalloon() {
  const layer = ensureFinalVictoryBalloonLayer();
  if (!layer) return;

  const balloonSources = ["./hab_orange.png", "./hab_pomelo.png", "./hab_lime.png"];
  const balloon = document.createElement("img");
  balloon.className = "final-victory-balloon";
  balloon.src = balloonSources[Math.floor(Math.random() * balloonSources.length)];
  balloon.alt = "";
  balloon.setAttribute("aria-hidden", "true");

  const sizeVh = 10 + Math.random() * 5;
  const travelVh = 78 + Math.random() * 12;
  const driftVw = (Math.random() * 2 - 1) * 10;
  const durationMs = 4200 + Math.random() * 1200;

  balloon.style.left = `${Math.random() * 100}vw`;
  balloon.style.bottom = `-${sizeVh}dvh`;
  balloon.style.width = `${sizeVh}dvh`;
  balloon.style.animationDuration = `${durationMs}ms`;
  balloon.style.setProperty("--balloonTravel", `${travelVh}dvh`);
  balloon.style.setProperty("--balloonDrift", `${driftVw}vw`);
...
function startFinalVictoryBalloonEffects(starCount) {
  ...
  const runId = finalVictoryBalloonRunId;
  const balloonCount = getFinalVictoryBalloonCount(starCount);
  const spawnIntervalMs = Math.max(250, Math.floor(5000 / balloonCount));

  const spawn = () => {
    if (runId !== finalVictoryBalloonRunId) return;
    spawnFinalVictoryBalloon();
  };

  spawn();
  let spawned = 1;
  finalVictoryBalloonSpawnTimer = window.setInterval(() => {
    if (runId !== finalVictoryBalloonRunId) return;
  ...
  finalVictoryBalloonStopTimer = window.setTimeout(() => {
    if (runId !== finalVictoryBalloonRunId) return;
    if (finalVictoryBalloonSpawnTimer != null) {
      window.clearInterval(finalVictoryBalloonSpawnTimer);
      finalVictoryBalloonSpawnTimer = null;
