...
const requiredAssetKeys = ["fieldBg", "tree", "leaf"];
  ...
  renderNode(node) {
    ...
    this.clearContent();
      ...
      if (node.type === "Text") {
        ...
        // Run optional event handler for the final text node (e.g., sound effect).
        if (node.Event && typeof window[node.Event] === "function") {
          try {
            window[node.Event](node);
          } catch (err) {
            console.error("Error running dialog event handler on final node", node.Event, err);
          }
        }

        // Automatically finish the dialog sequence when the final text is shown.
        this.stop();
        document.dispatchEvent(
          new CustomEvent("dialogSequenceCompleted", { detail: { node } })
        );
      }
      ...
      const count = Math.min(expectedCount, answers.length);
        ...
        btn.appendChild(frontSpan);
          ...
          // As soon as any choice is selected, make all buttons in this dialog
          // unresponsive so the player cannot click multiple answers.
          const allButtons = answersWrapper.querySelectorAll(".dialog-choice-button");
          allButtons.forEach((choiceBtn) => {
...
function startMenuDialogSystem() {
  ...
  const introDialogScript = [
    ...
    {
      type: "Choice",
      Question: "你想在哪一個範圍夾寶物？",
      AnswerNo: 1,
      AnswerArr: ["Level 1: 0 至 20"], /*"0至10 場地"*/
    },
    /*{
      type: "Choice",
      Question: "你想用哪一個夾子？",
...
// When the dialog script finishes (after the final TextFinal node),
// show a zooming helper bear image, then redirect to the game page
// once the enlargement animation has completed.
document.addEventListener("dialogSequenceCompleted", () => {
  ...
  // Small pause/freeze after the last line of dialog.
  setTimeout(() => {
    ...
    bear.style.left = "50%";
    bear.style.top = "50%";
    // Start very small so that scaling to 1 is effectively 20x enlargement.
    ...
    bear.style.transition = "transform 3000ms ease-in";
...
window.onload = async function() {
  ...
  const animatedIntroVideo = document.getElementById("helperBearAnimated");
  const staticHelperVideo = document.getElementById("helperBearStatic");
  ...
  let helperCrossfadeStarted = false;

  const startHelperCrossfade = () => {
    if (helperCrossfadeStarted) return;
    helperCrossfadeStarted = true;

    // Fade out the intro video
    if (animatedIntroVideo) {
      animatedIntroVideo.style.opacity = "0";
      setTimeout(() => {
        animatedIntroVideo.style.display = "none";
      }, 1000);
    }

    // Fade in the looping helper video (static/idle)
    if (staticHelperVideo) {
      staticHelperVideo.style.visibility = "visible";
      staticHelperVideo.style.opacity = "0";
      staticHelperVideo.currentTime = 0;
      staticHelperVideo.play().catch(() => {});

      requestAnimationFrame(() => {
        staticHelperVideo.style.opacity = "1";
      });
    }
  };
  ...
  const startTitleIntro = () => {
    ...
    // 使用原來的飛入動畫，僅在視口回到 scale(1) 之後才開始
    requestAnimationFrame(() => {
 ...
 const startTitleExitAndMenu = () => {
    ...
    const handleFlyOutEnd = (event) => {
      ...
      // 2 秒後（標題飛出完成後的既有流程），隱藏標題並顯示左側菜單標籤
      setTimeout(() => {
        ...
        if (menuContainer) {
          ...
          if (infoBox && menuButtons.length > 0) {
            const lastDelayMs = (menuButtons.length - 1) * 150;
  ...
  // 設定影片來源與播放邏輯（不再使用跑步動畫，只播放固定位置影片）
  if (animatedIntroVideo) {
    let introStarted = false;
    const introVideoSrc = "./stb_helper_animated.webm";

    if (!animatedIntroVideo.src || !animatedIntroVideo.src.includes("strawberry_helper_vid.webm")) {
      animatedIntroVideo.src = "./strawberry_helper_vid.webm";
    }
    ...
    const startIntroVideo = () => {
      ...
      const playPromise = animatedIntroVideo.play();
      ...
      // Prepare the static helper video but keep it hidden until crossfade
      if (staticHelperVideo && !staticHelperVideo.src) {
        staticHelperVideo.src = introVideoSrc;
        staticHelperVideo.style.opacity = "0";
        staticHelperVideo.style.visibility = "hidden";
        staticHelperVideo.load();
      }
    };

    animatedIntroVideo.addEventListener("loadeddata", startIntroVideo, { once: true });
    animatedIntroVideo.addEventListener("canplay", startIntroVideo, { once: true });

    if (animatedIntroVideo.readyState >= HTMLMediaElement.HAVE_CURRENT_DATA) {
      startIntroVideo();
    } else {
      animatedIntroVideo.load();
    }

    animatedIntroVideo.addEventListener("ended", () => {
      // Crossfade to the looping helper video
      startHelperCrossfade();

      // 視口從放大狀態平滑回到 scale(1)
      if (viewportContainer && viewportContainer.classList.contains("zoomed-in")) {
        const handleViewportTransitionEnd = (event) => {
          if (event.propertyName === "transform") {
            viewportContainer.removeEventListener("transitionend", handleViewportTransitionEnd);
            // 視口回到原始比例後，才讓標題進場
            startTitleIntro();
          }
        };

        viewportContainer.addEventListener("transitionend", handleViewportTransitionEnd);
        // 觸發縮放回原始視圖
        viewportContainer.classList.remove("zoomed-in");
      } else {
        // 如果沒有 zoomed-in 狀態，直接顯示標題
        startTitleIntro();
      }
    });
  } else {
    // 如果沒有 intro 影片，直接顯示標題與菜單序列
    startTitleIntro();
  }
    ...
    // Once the menu has finished its intro sequence, hide the prep overlay.
