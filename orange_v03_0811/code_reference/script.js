(() => {
  ...
  const END_MUSIC_HUMAN_WIN = "Gold_Medal_Run_loop.mp3";
  ...
  function animateEndScoreCount(side, targetScore, durationMs = 3000) {
  ...
  function startEndScoreCountUp() {
    animateEndScoreCount("left", state.leftScore, 3000);
    animateEndScoreCount("right", state.rightScore, 3000);
  }
  ...
  function setEndBearSpriteFrame(bear, frameIndex) {
    const columns = END_BEAR_SPRITE_COLUMNS;
    const col = frameIndex % columns;
    const row = Math.floor(frameIndex / columns);
    bear.style.backgroundPosition = `calc(-1 * ${col} * var(--end-bear-render-size)) calc(-1 * ${row} * var(--end-bear-render-size))`;
  }

  function startEndBearSpriteLoop(side, imageUrl, frameSequence) {
    const bear = getEndBearBySide(side);
    ...
    bear.dataset.spriteColumns = String(END_BEAR_SPRITE_COLUMNS);
    bear.style.backgroundImage = `url("${imageUrl}")`;
    bear.style.backgroundRepeat = "no-repeat";
    bear.style.backgroundSize = `calc(${END_BEAR_SPRITE_COLUMNS} * var(--end-bear-render-size)) calc(${END_BEAR_SPRITE_COLUMNS} * var(--end-bear-render-size))`;

    let frameCursor = 0;
    setEndBearSpriteFrame(bear, frameSequence[frameCursor]);
  ...
  function applyMultiplayerRankBears(rankResult) {
    ...
    if (rankResult.tie) {
      ...
      startEndBearSpriteLoop("right", END_FIRST_BEAR_SRC_RIGHT_MULTIPLAYER, END_FIRST_BEAR_LOOP_FRAMES);
      return;
    }
  ...
  function showPassTurnOverlayAfterSideAnimation(side) {
    ...
    const waitMs = getBearJumpRemainingMs(side);
    if (waitMs <= 0) {
      showPassTurnOverlay();
      return;
    }
  ...
  function init() {
    ...
    ensureEndBearLayout();
    if (ui.endOverlay) {
      ui.endOverlay.style.setProperty("--end-bear-scale", String(END_BEAR_SIZE_SCALE));
    }
